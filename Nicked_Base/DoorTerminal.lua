...
local devices = {
  [1] = {
    id = 1,
    openCommandId = "example.open",
    closeCommandId = "example.close",
    openText = "open example",
    closeText = "close example",
  },
  [2] = {
    id = 2,
    openCommandId = "example2.open",
    closeCommandId = "example2.close",
    openText = "open example2",
    closeText = "close example2",
  },
}
...
if input == password then
        ...
        while true do
                ...
                for _, d in ipairs(devices) do
                        if input == d.openText then
                                local tId, cmd = d.id, d.openCommandId
                                Communication(tId, cmd)
                                handled = true
                                break
                        elseif input == d.closeText then
                                local tId, cmd = d.id, d.closeCommandId
                                Communication(tId, cmd)
                                handled = true
                                break
                        end
                ...
                if not handled then
                        if input == "call_elevator" then
                                rs.setOutput(rsSide, true)
                                sleep(0.5)
                                rs.setOutput(rsSide, false)
                                animateDots(19, 13, "Calling", 5, color)
                                sleep(readTime)
